...
##print(html_text)
# Only the bold URL can change, everything else in that ^ line must stay exactly the same!
# Grabbed the data at http://si.umich.edu place on the internet, get the text of it into the html_text variable
f = io.open("si_html_text.html", "w", encoding="utf-8") # Open a file to write to, call it si_html_text.html -- it's gonna be an html type file
f.write(html_text) # Write the string of the html data to the file you opened
##fileref.close()
...
## Use the requests library (or other Python code that has the same effect) and Beautiful Soup to gather, from that web page, each of the 
##names and the titles. You should create a dictionary called umsi_titles which contains the names and the titles for each person on that page. 
##For example, your dictionary should include the following key-value pairs (and a bunch more...):
...
sou = BeautifulSoup(htmldoc,"html.parser")
people = sou.find_all("div",{"class":"views-row"})
titl = sou.find_all(property="dc:title")
print(titl)
umsi_titles = {}
